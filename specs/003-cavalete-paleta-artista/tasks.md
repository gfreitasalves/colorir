...
- [X] T001 [P] Em `src/index.css`, adicionar as variáveis `--studio-wood-base`, `--studio-wood-dark`
...
- [X] T002 [P] Criar `src/components/icons/StudioDecorIcons.jsx` (mesmo padrão de
...
- [X] T003 Em `src/index.css`, adicionar as classes utilitárias `.studio-wood-surface` (gradiente de
...
- [X] T004 [US1] Em `src/components/icons/StudioDecorIcons.jsx`, adicionar os componentes
  ...
  de T002.
- [X] T005 [US1] Em `src/components/Canvas.jsx`, envolver o `<div ref={scrollRef}>` existente (que
...
- [X] T006 [US1] Ajustar a decoração de T005 para caber em telas pequenas: simplificar/ocultar
...
- [X] T007 [US1] Validar manualmente o Cenário 1 de `quickstart.md` em desktop e mobile: abrir um
...
- [X] T008 [P] [US2] Em `src/components/icons/StudioDecorIcons.jsx`, adicionar o componente
...
- [X] T009 [US2] Em `src/components/Palette.jsx`, envolver o bloco existente da seção "Paleta de
...
- [X] T010 [US2] Em `src/components/Palette.jsx`, atualizar o componente `Swatch` (linhas 6-19) para
...
- [X] T011 [US2] Em `src/components/Palette.jsx`, dar um acabamento visual de ateliê aos elementos
...
- [X] T012 [US2] Confirmar que a seção "Adesivos" (quando essa ferramenta está ativa, linhas 88-107
...
- [X] T013 [US2] Validar manualmente o Cenário 2 de `quickstart.md` em desktop e mobile: escolher
...
- [X] T014 [P] Validar manualmente o Edge Case de `quickstart.md` (Cenário 3): "Ações", "Ferramenta",
  "Zoom" e "Terminei!" continuam presentes e funcionando normalmente, fora da área estilizada como
  paleta (FR-005).
- [X] T015 [P] Validar manualmente o Cenário 4 de `quickstart.md`: alternar entre os 4 temas de
  ...
  `research.md`).
- [X] T016 [P] Criar `docs/estudio-cavalete-paleta.md` documentando a feature (cavalete, paleta de
...
- [X] T017 Atualizar `docs/painel-lateral.md` para descrever a nova aparência de paleta de artista na
  seção de cores (a ordem das seções, definida na feature 002, continua a mesma — só a aparência da
  seção de cores muda).
- [X] T018 [P] Adicionar link para `docs/estudio-cavalete-paleta.md` em `docs/README.md` e um item na
  lista de funcionalidades de `README.md` (seção `#funcionalidades`).
- [X] T019 Rodar `npm run build` e corrigir qualquer erro antes de prosseguir.
- [X] T020 Rodar `npm run preview` e validar manualmente TODOS os cenários de `quickstart.md`
  (incluindo a seção "Validação mobile") em viewport desktop e mobile.
- [X] T021 Commit na branch de trabalho e push, seguindo o fluxo de Git/PR do projeto
  (`docs/deploy.md`).
