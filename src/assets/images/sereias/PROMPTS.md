# Prompts prontos — Sereias

Cada bloco abaixo já é o prompt completo (base + assunto), pronto para colar direto no Gemini (ou outra ferramenta). Veja [`../PROMPT-BASE-GERACAO.md`](../PROMPT-BASE-GERACAO.md) para o porquê de cada regra.
...
SUBJECT:
A cute cartoon mermaid girl waving, sitting on a rock by the sea
```
...
SUBJECT:
A mermaid swimming alongside a friendly dolphin
```
...
SUBJECT:
A mermaid princess with a crown and a long scaled tail
```
...
SUBJECT:
Two mermaid friends playing with a starfish
```
...
SUBJECT:
A mermaid with flowing hair holding a large seashell
```
...
SUBJECT:
A baby mermaid with a small fish friend
```
